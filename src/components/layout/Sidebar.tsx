  ...
  return (
    <aside className="border-b border-slate-800 bg-[#101318] md:fixed md:inset-y-0 md:left-0 md:z-20 md:w-64 md:border-r md:border-b-0">
      <div className="flex h-16 items-center border-b border-slate-800 px-5">
        <div className="flex h-8 w-8 items-center justify-center rounded-md border border-cyan-700/60 bg-cyan-950/50 text-sm font-bold text-cyan-300">
          B
        </div>
      ...
      >
        {navigation.map((item) => {
          const active =
            currentPage === item.id ||
            (currentPage === "editor" && item.id === "reports");
          return (
            <button
