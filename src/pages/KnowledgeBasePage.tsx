  ...
  return (
    ...
    >
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        {textList("indicators", "Common indicators")}
        {textList("evidenceChecklist", "Evidence collection checklist")}
        {textList("reportQuestions", "Questions to answer")}
        {textList("suggestedSections", "Suggested report sections")}
        {textList("tags", "Related tags")}
        <label className="field-group">
          <span>Impact guidance</span>
          <textarea
            className="input-field min-h-20 resize-y"
            value={draft.impactGuidance}
            onChange={(event) =>
              setDraft({ ...draft, impactGuidance: event.target.value })
            }
          />
        </label>
        <label className="field-group">
          ...
          <textarea
            ...
            onChange={(event) =>
              setDraft({ ...draft, remediationGuidance: event.target.value })
            }
          />
        </label>
      </div>
...
export function KnowledgeBasePage({
  ...
  const [selected, setSelected] = useState<KnowledgeEntry>();
  const [editing, setEditing] = useState<KnowledgeEntry>();
  const [deleting, setDeleting] = useState<KnowledgeEntry>();
  const entries = useMemo(
    () => [...BUILT_IN_KNOWLEDGE, ...customEntries],
    [customEntries],
  );
    ...
    () =>
      entries.filter(
        (entry) =>
          ...
          (!search ||
            `${entry.name} ${entry.definition} ${entry.tags.join(" ")}`
              .toLocaleLowerCase()
              .includes(search.toLocaleLowerCase())),
      ),
    [category, entries, search],
  );
  const copyPrompts = async (entry: KnowledgeEntry) => {
    ...
    try {
      ...
      onNotify("success", "Writing guidance copied.");
    } catch {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="flex flex-wrap items-end gap-3 rounded-lg border border-slate-800 bg-[#101318] p-4">
        <label className="field-group min-w-56 flex-1">
          <span>Search knowledge base</span>
          <input
            className="input-field"
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Search definitions, categories, tags"
          />
        </label>
          ...
          >
            <div className="flex justify-between gap-3">
              ...
              <span className="badge border-slate-700 bg-slate-800 text-slate-400">
                {entry.isBuiltIn ? "Built-in" : "Custom"}
              </span>
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <button
                className="button-secondary px-3 py-1.5 text-xs"
                type="button"
                onClick={() => void copyPrompts(entry)}
              >
                Copy Prompts
              </button>
              {!entry.isBuiltIn && (
      ...
      {selected && (
        ...
        >
          <section
            className="max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-lg border border-slate-700 bg-[#161a20] p-6"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            <div className="mt-5 space-y-5 text-sm">
              <div>
                <h3 className="font-medium text-slate-200">Definition</h3>
              ...
              <div>
                <h3 className="font-medium text-slate-200">Impact guidance</h3>
                <p className="mt-1 text-slate-400">{selected.impactGuidance}</p>
              </div>
              <div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button
                ...
                type="button"
                onClick={() => void copyPrompts(selected)}
              >
      ...
      <ConfirmDialog
        ...
        title="Delete custom knowledge entry?"
        description={`This removes “${deleting?.name ?? "this entry"}” from this browser.`}
