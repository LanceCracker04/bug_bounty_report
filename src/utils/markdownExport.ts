...
export function createReportMarkdown(
  ...
  const evidence = report.evidenceItems
    .map(
      (item) =>
        `- **${item.title}** (${item.type})${item.description && preferences.includeEvidenceDescriptions ? `: ${item.description}` : ""}${item.sourceUrl ? ` — ${item.sourceUrl}` : ""}`,
    )
    .join("\n");
  const references = report.references
    .map((item) => `- [${item.label}](${item.url})`)
    .join("\n");
  const timeline = report.disclosureTimeline
    .map((item) => `- ${item.date ? `${item.date}: ` : ""}${item.event}`)
    .join("\n");
  return (
