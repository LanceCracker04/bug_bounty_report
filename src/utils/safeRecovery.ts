...
export interface StorageKeyStatus {
  key: string;
  state: "Missing" | "Valid JSON" | "Malformed JSON" | "Unavailable";
}
export function storageKeyStatus(): StorageKeyStatus[] {
  return [REPORT_STORAGE_KEY, ...optionalKeys].map((key) => {
    try {
      ...
      try {
        JSON.parse(raw);
        return { key, state: "Valid JSON" as const };
      } catch {
        return { key, state: "Malformed JSON" as const };
      }
    } catch {
...
export function shouldStartSafeRecovery(): boolean {
  return storageKeyStatus().some((item) => item.state === "Malformed JSON");
}
export function exportRawMetadataBackup(): void {
  const data = Object.fromEntries(
    storageKeyStatus().map(({ key, state }) => {
      try {
        return [key, { state, value: localStorage.getItem(key) }];
      } catch {
        return [key, { state, value: null }];
      }
    }),
  );
  ...
  link.href = url;
  link.download = "bug-bounty-report-raw-metadata-recovery.json";
